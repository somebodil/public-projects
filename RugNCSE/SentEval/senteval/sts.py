    ...
    def run(self, params, batcher):
        ...
        for dataset in self.datasets:
            ...
            results[dataset] = {
                ...
                ###### newly added by yyh ######
                'align_loss': np.mean(all_loss_align, dtype=np.float64),
                'uniform_loss': np.mean(all_loss_uniform, dtype=np.float64),
                ################################
            }

            logging.debug(
                '%s : pearson = %.4f, spearman = %.4f, align_loss = %.4f, uniform_loss = %.4f' % (
                    dataset,
